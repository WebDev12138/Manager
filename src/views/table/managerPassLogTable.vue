  ...
  <div class="app-container">
    <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
      <el-form :inline="true">
        ...
        <el-form-item >
          <el-input placeholder="管理员ID,审核人" v-model="searchName" clearable @change="search"></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="tableList" v-loading="listLoading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column prop="manager_id" min-width="200px" label="管理员ID">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    fetchData() {
      ...
      axios.post('http://118.31.102.1:9200/passLog/managerPassLog', { pageNum: this.pageSize, pageSize: this.pageSize }).then(response => {
    ...
    getAllData() {
      axios.get('http://118.31.102.1:9200/passLog/getAllManager').then(res => {
        this.all = res.data
      }).catch((error) => {
        Message.error(error.response.data.message)
      })
    ...
    doFilter() {
      ...
      this.all.forEach((value, index) => {
        if (value.manager_id) {
          if (value.manager_id.indexOf(this.searchName) >= 0) {
