...
export default {
  ...
  methods: {
    fetchData() {
      ...
      axios.get('http://localhost:9200/role/getUserRole?pageNum=' + this.page + '&pageSize=' + this.pageSize).then(response => {
        // const limit = 10
    ...
    updateData(formName) {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          axios.post('http://localhost:9200/role/updateUserRole', { managerId: this.edit.manager_user_id, roleId: this.edit.role_id }).then(() => {
